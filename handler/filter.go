...
func Filter(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		//判断是否未管理员
		org := GetOrgByAccountId(convert.MustInt64(m[0]["id"]))
		if org == nil {
			return utils.AuthFail(c, "无后台管理权限")
		}
		c.Set("account", global.ToMapAccount(m[0]))
		c.Set("org", global.ToMapOrg(org))
		return next(c)
	}
}
...
func GetOrg(c echo.Context) (org *global.Org, err error) {
	itf := c.Get("org")
	if itf == nil {
		err = utils.AuthFailOrgNull(c)
		return
	}
	org = global.ToInterfaceOrg(itf)
	if org == nil {
		err = utils.AuthFailOrgNull(c)
		return
	}
