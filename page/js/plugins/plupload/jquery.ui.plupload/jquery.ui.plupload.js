    ...
    $.widget("ui.plupload", {
        ...
        _initUploader: function () {
            ...
            uploader.bind('FileUploaded', function (up, file, r) {
                ...
                var json = eval("[" + r.response + "]")[0];
                if (json.Key != "error") {
                    imgsrcArr = new Array();
                    var isrcs = $("#hidImageSrcs").val();
                    if (isrcs != "") {
                        var str = isrcs.split(",");
                        for (var i = 0; i < str.length; i++) {
                            imgsrcArr.push(str[i]);
                        }
                    }
                    imgsrcArr.push(json.Value);
                    $("#hidImageSrcs").val(imgsrcArr.join(","));
                    //var wxh = obj.toString().split('~')[1].split('.')[0].split('x');
